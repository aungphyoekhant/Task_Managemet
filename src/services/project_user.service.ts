...
export const projectUserService = {
  ...
  addMember: async (
    ...
    const result = await prisma.$transaction(async (tx) => {
      const projectUsers = await Promise.all(
        createData.map((data) => tx.projectUser.create({ data }))
      );
  ...
  getMembersByProject : async (workspaceId : number,projectId: number, userId: number) => {
    ...
    removeMember: async (workspaceId: number, projectId: number, projectUserId: number, userId: number) => {
