...
import { TaskStatus, ProjectStatus } from "../../generated/prisma/client.js";
...
export const dashboardService = {
    ...
    getDashboardStats: async ({ userId, workspaceId, role }) => {
        ...
        const projectSummaries = projects.map((project) => {
            ...
            if (projectTotalTasks > 0) {
                const totalWeight = project.tasks.reduce((sum, task) => sum + TASK_STATUS_WEIGHT[task.status], 0);
    ...
    getAllWorkspacesStats: async (userId) => {
        ...
        const [totalWorkspaces, totalProjects, totalTasks, totalWorkspaceMembers, totalProjectMembers, totalTaskUsers, 
        // Task Status Summaries
        todoTasks, inProgressTasks, doneTasks, overdueTasks, 
        // Project Status Summaries
        pendingProjects, activeProjects, completedProjects, 
        // Workspace-wise Details with nested breakdown
        workspacesData,] = await Promise.all([
        ...
        const overallTaskCompletionRate = totalTasks > 0 ? Math.round((doneTasks / totalTasks) * 100) : 0;
