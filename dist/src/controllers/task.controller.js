...
export const taskController = {
    createTask: async (req, res) => {
        try {
            const { projectId, title, description, priority, status, dueDate, workspaceId } = req.body;
    ...
    getTasks: async (req, res) => {
        try {
            ...
            const result = await taskService.getTasks(Number(workspaceId), Number(projectId), Number(userId), cursor ? Number(cursor) : undefined, limit ? Number(limit) : 10, status ? status : undefined);
            return res.status(200).json({ con: true, ...result });
        }
        catch (error) {
            console.error("--> GetTasks Error Details:", error);
            return res.status(500).json({ con: false, msg: error.message });
        }
    },
    ...
    updateAssignedTask: async (req, res) => {
        try {
            ...
            if (!task)
                return res.status(404).json({ con: false, msg: "Task not found" });
            // ✅ Assign လုပ်ထားခြင်း ရှိမဖြစ်ကို isUserAssignedToTask ဖြင့် စစ်ဆေးခြင်း
            const isAssigned = await taskService.isUserAssignedToTask(taskId, userId);
            if (!isAssigned) {
                return res.status(403).json({ con: false, msg: "Not assigned to this task" });
            }
            // ✅ တကယ် update လုပ်မည့် service ကို arguments ၄ ခုဖြင့် တိကျစွာ ခေါ်သုံးခြင်း
            const updated = await taskService.updateAssignedTask(taskId, value, userId, Number(task.workspaceId));
    ...
    deleteTask: async (req, res) => {
        try {
            ...
            return res.status(200).json({ con: true, msg: "Deleted Success" });
        }
        catch (error) {
            return res.status(500).json({ con: false, msg: error.message });
        }
