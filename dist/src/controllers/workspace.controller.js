...
export const workspaceController = {
    ...
    getAllWorkspaceByUserId: async (req, res) => {
        try {
            const userId = Number(res.locals.user.id);
            if (!userId) {
    ...
    createWorkspace: async (req, res) => {
        ...
        try {
            const workspace = await workspaceService.createWorkspace(value.userId, value.name, value.logo);
            res.status(201).json({
                con: true,
    ...
    modifyWorkspace: async (req, res) => {
        ...
        try {
            ...
            const updateData = { name: value.name };
